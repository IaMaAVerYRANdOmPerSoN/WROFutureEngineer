    ...
    async with Client() as client, OpenChallengeAsyncMultiprocessingVisionProcessor() as vision, AsyncCamera() as camera:
        ...
        try:
            ...
            async for zone, walls, corner_lines, wall_x_diffs in OpenChallengeAsyncMultiprocessingVisionProcessor.async_pipe_reader(data_receiver):
                ...
                assert isinstance(corner_lines, tuple) and all(isinstance(line, VisionObject) for line in corner_lines) or not corner_lines # Empty tuple

                logger.debug(f"Zone: {zone}, Walls: {walls}, Corner Lines: {corner_lines}, Wall Dists: {wall_x_diffs}")
                state_history[1] += 1 if state_history[0] == state else 0
                ...
                match state:
                    ...
                    case "Turn":
                        if not corner_lines:
                            ...
                            state = "Follow wall"
                        elif not any(isinf(x_diff) for x_diff in wall_x_diffs.values()): # We know that the corner lines are still good from the previous condition, so we can use hybrid mode
                            wall_follow.previous_error = 0
                            state = "hybrid"
                        else:
                        ...
                        if not corner_lines:
                            ...
                            turn_correction = (
                                ...
                                + corner_turn_controller.tick(
                                    ...
                                    - Config.CameraConfig.FORMAT["size"][0] // 2) # Biggest corner line x - frame center
                                // 2) # Average
                            asyncio.create_task(
                                client.drive_motors(
