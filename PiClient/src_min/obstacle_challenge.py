from typing import Literal
from src_min import logger
from .camera import Camera
from .commProtocol import Client
from .controller import PD
from .visionProcessing import VisionObject, VisionProcessor
from .config import Config


def run_obstacle_challenge():
    vision = VisionProcessor()
    with Client() as client, Camera() as camera:
        wall_follow = PD(*Config.ObstacleChallengeConfig.WALL_FOLLOW_KPKD)
        obstacle_avoid = PD(
            *Config.ObstacleChallengeConfig.OBSTACLE_AVOID_KPKD)
        corner_turn_controller = PD(
            *Config.ObstacleChallengeConfig.CORNER_TURN_KPKD)

        state: Literal["Straight", "Turn"] = "Straight"

        if not client.verify_connection():
            return

        try:
            while True:
                frame = camera.get_frame()
                zone, walls, obstacles, corner_lines, wall_x_diffs, obstacle_dists, obstacle_path_x = vision.comprehensive_analysis(
                    frame
                )

                if zone is not None:
                    assert isinstance(zone, VisionObject)
                    assert isinstance(walls, tuple) and all(
                        isinstance(wall, VisionObject) for wall in walls
                    )
                    assert isinstance(obstacles, tuple) and all(
                        isinstance(obstacle, VisionObject) for obstacle in obstacles
                    )
                    assert isinstance(corner_lines, tuple) and all(
                        isinstance(line, VisionObject) for line in corner_lines
                    )

                logger.debug(
                    f"Zone: {zone}, Walls: {walls}, Obstacles: {obstacles}, Corner Lines: {corner_lines}, Wall Dists: {wall_x_diffs}, Obstacle Dists: {obstacle_dists}, Obstacle Path X: {obstacle_path_x}"
                )

                match state:
                    case "Straight":
                        if corner_lines:
                            state = "Turn"
                        elif obstacles:
                            turn_correction = obstacle_avoid.tick(
                                obstacle_path_x)
                            client.drive_motors(
                                Config.ObstacleChallengeConfig.DRIVE_SPEED,
                                turn_correction,
                                Config.ObstacleChallengeConfig.DRIVE_COMMAND_DURATION,
                            )
                        else:
                            turn_correction = wall_follow.tick(
                                wall_x_diffs["left"] - wall_x_diffs["right"]
                            )
                            client.drive_motors(
                                Config.ObstacleChallengeConfig.DRIVE_SPEED,
                                turn_correction,
                                Config.ObstacleChallengeConfig.DRIVE_COMMAND_DURATION,
                            )

                    case "Turn":
                        if not corner_lines:
                            state = "Straight"
                        else:
                            turn_correction = corner_turn_controller.tick(
                                corner_lines[0].x_centroid
                                - Config.CameraConfig.FORMAT["size"][0] // 2
                            )
                            client.drive_motors(
                                Config.ObstacleChallengeConfig.DRIVE_SPEED,
                                turn_correction,
                                Config.ObstacleChallengeConfig.DRIVE_COMMAND_DURATION,
                            )

                    case _:
                        logger.error(f"Invalid state {state}")
                        state = "Straight"
        finally:
            logger.info("Stopping...")
            client.drive_motors(0, 0, 0)
            logger.success("Done")
