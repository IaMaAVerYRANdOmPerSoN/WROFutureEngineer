...
async def test_single_frame():
    ...
    async with AsyncCamera() as camera:
        ...
        assert frame.shape == (96, 128, 3), f"Incorrect shape: {frame.shape}"
...
async def test_streaming_pipe():
    ...
    shm = shared_memory.SharedMemory("frameBuffer", create=True, size= 128*96*3 + 100)
    ...
    try:
        ...
        async for result in AsyncCamera.frame_yeilder(receiver): # Now a classmethod!
            if result == True:
                ...
                frame_counter += 1
                if frame_counter >= 30:
                    logger.success(f"Sucessfully fetched 30 frames: Average Framerate {1/(time.time()-start) * 30:.2f}")
                    return True
